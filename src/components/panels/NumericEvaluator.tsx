...
export default function NumericEvaluator() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setCalcBlocks([...calcBlocks, <CalcBlock expression={value} solution={evaluate(value)} />]);
  }

  return (
    <div>
      <div id={styles.panelContainer}>
        {...calcBlocks}
      </div>
      <form onSubmit={handleSubmit}>
        <input id={"expressionField"} className={styles.expressionField} />
      </form>
    </div>
  )
}